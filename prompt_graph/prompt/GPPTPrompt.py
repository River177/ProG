...
class SimpleMeanConv(MessagePassing):
    def __init__(self):
        ...
        super().__init__(aggr="mean")  # 'mean'聚合。
...
class GPPTPrompt(torch.nn.Module):
    def __init__(self, n_hidden, center_num, n_classes, device):
        ...
        for i in range(center_num):
            self.TaskToken.append(
                torch.nn.Linear(2 * n_hidden, n_classes, bias=False)
            )  # task token
    ...
    def update_StructureToken_weight(self, h):
        ...
        if h.size(0) > 20000:
            cluster_ids_x, cluster_centers = kmeans(
                X=h, num_clusters=self.center_num, distance="euclidean", device=self.device
            )
